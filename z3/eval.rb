...
dl_files.each do |dlFile|
    ...
    puts "Creating datalog specification .."
    ...
    # adapt facts for z3
    puts "Appending facts to datalog specification .."
    File.open("#{name}.dl", 'a') { |file|
        if File.exists?(facts) then
            Find.find(facts) do |path|
                next unless path =~ /.*\.facts$/ 
                rel_name = File.basename(path,".facts")
